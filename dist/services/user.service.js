...
exports.userService = {
    getMe: async (userId, tenantId) => {
        const user = await UserRepository_repo_1.userRepository.findById(userId, tenantId);
        if (!user) {
    ...
    getUsers: async (tenantId) => {
        return await UserRepository_repo_1.userRepository.findByTenant(tenantId);
    },
    createUser: async (data, tenantId) => {
        const hashedPassword = await (0, password_1.hashPassword)(data.password);
        return await UserRepository_repo_1.userRepository.createUser({
            ...data,
